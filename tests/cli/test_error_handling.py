...
class TestNiveshPyCommandErrorHandling:
    ...
    def test_catches_niveshpy_error(self, runner):
        ...
        with patch("niveshpy.cli.utils.overrides.output") as mock_output:
            result = runner.invoke(cmd)
        assert result.exit_code == 1
        mock_output.handle_error.assert_called_once()
    ...
    def test_error_message_in_handle_error(self, runner):
        ...
        with patch("niveshpy.cli.utils.overrides.output") as mock_output:
            runner.invoke(cmd)
        passed_error = mock_output.handle_error.call_args[0][0]
    ...
    def test_exit_code_is_1(self, runner):
        ...
        with patch("niveshpy.cli.utils.overrides.output"):
            result = runner.invoke(cmd)
        assert result.exit_code == 1
