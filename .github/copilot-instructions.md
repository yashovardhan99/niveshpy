...
```
CLI (Click commands) → Services (business logic) → Database (SQLModel/SQLite)
...
- **Services layer** (`niveshpy/services/`): One service per domain entity. Services accept query strings, translate them into SQL filters via the query language, and use `database.get_session()` for DB access.
- **Models** (`niveshpy/models/`): SQLModel models with a layered pattern — `FooBase` (shared fields) → `FooCreate` (input without ID) → `Foo(table=True)` (DB model) → `FooPublic` (output with ID). Domain models: `Account`, `Security`, `Transaction`, `Price`.
...
- **Database** (`niveshpy/database.py`): SQLite via SQLModel/SQLAlchemy. Stored in `platformdirs.user_data_path("niveshpy")`. Registers a custom `iregexp` SQLite function and enables foreign keys on every connection.
...
- **Session management** — always use `with get_session() as session:` for database access. Never hold sessions across operations.
